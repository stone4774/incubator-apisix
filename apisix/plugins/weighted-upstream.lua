...
function _M.init()
    core.log.info("[test] plugin balancer init, plugin_name: ",plugin_name)
...
local function create_upstream_picker(conf)
    ...
    core.log.info("[test] plugin balancer: ", plugin_name,", create_upstream_picker  conf: ", core.json.encode(conf))
    ...
    return {
        get = function ()
            core.log.info("[test] plugin balancer: ", plugin_name,", create_upstream_picker picker.get  conf: ", core.json.encode(conf))
            return picker:find()
        end
    }
end
...
function _M.balancer(conf, ctx)
    core.log.info("[test] plugin balancer: ", plugin_name, ", ctx.conf_id: ", ctx.conf_id," ,ctx.conf_version: ",ctx.conf_version, ", conf: ", core.json.encode(conf))

    --
    local key = ctx.conf_type .. "#" .. ctx.conf_id

    core.log.info("[test] plugin balancer: ", plugin_name, ", cachekey: ", key, ", conf: ", core.json.encode(conf))
    ...
    core.log.info("[test] plugin balancer: ", plugin_name, ", ip: ", ip, ", port: ", port)
